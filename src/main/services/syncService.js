...
class SyncService {
  ...
  _localHasChangedSince(lastSyncAt) {
    ...
    try {
      if (this.db.prepare("SELECT 1 FROM todos WHERE updated_at > ? LIMIT 1").get(lastSyncAt)) return true;
      if (this.db.prepare("SELECT 1 FROM subtasks WHERE created_at > ? LIMIT 1").get(lastSyncAt)) return true;
      if (this.db.prepare("SELECT 1 FROM reviews WHERE created_at > ? LIMIT 1").get(lastSyncAt)) return true;
    } catch {
      return true;
    }
  ...
  _mergeRemoteIntoLocal(remote) {
    ...
    const tx = this.db.transaction(() => {
      ...
      const localTodosMap = new Map(
        this.db.prepare("SELECT id, updated_at FROM todos").all().map((r) => [r.id, r]),
      );
      ...
      const localSubIds = new Set(
        this.db.prepare("SELECT id FROM subtasks").all().map((r) => r.id),
      );

      for (const rs of d.subtasks || []) {
        if (!localSubIds.has(rs.id)) {
          const todoExists = this.db.prepare("SELECT 1 FROM todos WHERE id = ?").get(rs.todo_id);
          if (todoExists) {
            this.db
              .prepare(
                `INSERT OR IGNORE INTO subtasks (id,todo_id,title,is_review,is_completed,sort_order,completed_at,created_at,deadline,tags)
      ...
      const localRevIds = new Set(
        this.db.prepare("SELECT id FROM reviews").all().map((r) => r.id),
      );

      for (const rr of d.reviews || []) {
        if (!localRevIds.has(rr.id)) {
          const todoExists = this.db.prepare("SELECT 1 FROM todos WHERE id = ?").get(rr.todo_id);
          if (todoExists) {
            this.db
              .prepare(
      ...
      const server = http.createServer((req, res) => {
        try {
          ...
          if (oauthError) {
            ...
            return finish(reject, new Error(`Dropbox OAuth error: ${oauthError}`));
          }
  ...
  async syncNow({ pullFirst = false } = {}) {
    ...
    try {
      ...
      if (remote) {
        ...
        const remoteChanged = !lastSyncAt || (remoteExportedAt && remoteExportedAt > lastSyncAt);
